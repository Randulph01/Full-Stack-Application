...
export Class PakedAccountInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<envp>> {
        ...
        function updateAccount() {
            if (!isAuthenticate()) return unauthorized();
            ...
            // don't save conflict password
            delete params.configPassword;
        ...
        function error(message) {
            return throwError({ error: { message }})
                ...
                // call materialize and dematerialize to ensure delay even if an error is thrown (https://)
        }
        ...
        function currentAccount() {
            ...
            const resetTokenExpired = Date.now() > (jwtToken.exp * 1000);
        ...
        function generateJwtToken(account) {
            ...
            const tokenPayload = {
                exp: Math.rount(new Date(Date.now() +15*60*1000).getTime() / 1000),
                id: account.id
            }
